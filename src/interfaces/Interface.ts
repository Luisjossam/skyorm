...
export interface ModelWhere {
   ...
   */
  where(conditions: Record<string, any>): MW;
}
export interface ModelOrWhere {
  orWhere(conditions: Record<string, any>): MOW;
}
...
export interface ModelOrderBy {
   ...
   */
  orderBy(column: string, sort: "ASC" | "DESC"): MOB;
}
export interface ModelSum {
  /**
   ...
   */
  sum(column: string, conditions?: Record<string, any>, alias?: string): MS;
}
export interface ModelCount {
  count(column?: string, conditions?: Record<string, any>, alias?: string): MC;
}
export interface ModelMin {
  min(column: string, conditions?: Record<string, any>, alias?: string): MMin;
}
export interface ModelMax {
  max(column: string, conditions?: Record<string, any>, alias?: string): MMax;
}
export interface ModelAvg {
  avg(column: string, conditions?: Record<string, any>, alias?: string): MA;
}
export interface ModelHaving {
  having(conditions: Record<string, any>): MH;
}
// FINALS METHODS
export interface ModelFind {
  /**
   ...
   */
  find(primary_key: number | string, columns?: string[]): Promise<Record<string, any>>;
}
export interface ModelGet {
  get(columns?: string[]): Promise<Record<string, any>>;
}
export interface ModelGetOne {
  /**
   ...
   * console.log(result); // Output: A single model instance with `id` and `name` columns or `null` if no record is found
   */
  getOne(columns?: string[]): Promise<Record<string, any>>;
}
export interface ModelGroupBy {
  groupBy(...columns: string[]): Promise<any[]>;
}
export interface ModelPluck {
  /**
...
export interface ModelPaginate {
   ...
   */
  paginate(current_page: number, per_page: number, columns?: string[]): Promise<IPaginateData>;
}
export interface ModelExist {
  /**
